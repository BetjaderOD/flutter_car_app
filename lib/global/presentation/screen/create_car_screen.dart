...
class CarCreateView extends StatelessWidget {
  final CarModel? car;

  const CarCreateView({Key? key, this.car}) : super(key: key);

  @override
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text(car == null ? 'Crear Carro' : 'Actualizar Carro'),
      ),
      body: BlocProvider(
        create: (context) => CarCubit(
          carRepository: RepositoryProvider.of<CarRepository>(context),
        ),
        child: CarCreateScreen(car: car),
      ),
    );
  }
}
      ...
      child: Column(
        children: [
          ...
          ElevatedButton(
            ...
            child: Text(widget.car == null ? 'Crear Carro' : 'Actualizar Carro'),
          ),
        ],
      ),
    );
